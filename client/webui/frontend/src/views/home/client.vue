...
import {apiClientCreate, apiClientDelete, apiClientList, apiClientStatus, apiClientUpdate} from "../../api/client.js";
...
const confirmCreateFunc = () => {
  createRef.value.validate(async valid => {
    if (!valid) {
      try {
        await apiClientCreate(state.value.create.data)
        closeCreateModal()
        await listFunc()
      } finally {
...
const confirmUpdateFunc = () => {
  updateRef.value.validate(async valid => {
    if (!valid) {
      try {
        await apiClientUpdate(state.value.update.data)
        closeUpdateModal()
        await listFunc()
      } finally {
...
const deleteFunc = async (row) => {
  try {
    ...
    await apiClientDelete({key: row.key})
    await listFunc()
  } finally {
