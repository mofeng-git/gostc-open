...
import {apiP2PCreate, apiP2PDelete, apiP2PList, apiP2PStatus, apiP2PUpdate} from "../../api/p2p.js";
...
const confirmCreateFunc = () => {
  createRef.value.validate(async valid => {
    if (!valid) {
      try {
        await apiP2PCreate(state.value.create.data)
        closeCreateModal()
        await listFunc()
      } finally {
...
const confirmUpdateFunc = () => {
  updateRef.value.validate(async valid => {
    if (!valid) {
      try {
        await apiP2PUpdate(state.value.update.data)
        closeUpdateModal()
        await listFunc()
      } finally {
...
const deleteFunc = async (row) => {
  try {
    ...
    await apiP2PDelete({key: row.key})
    await listFunc()
  } finally {
