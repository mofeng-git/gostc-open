...
import (
	...
	common2 "gostc-sub/internal/common"
	"sync"
	"time"
)
...
func (e *Event) OnOpen(socket *gws.Conn) {
	common2.Logger.AddLog(e.tp, "WS连接成功")
	go func() {
		for {
	...
	e.WriteAny(socket, common2.NewMessage(uuid.NewString(), "register", regData))
}

func (e *Event) OnClose(socket *gws.Conn, err error) {
	common2.Logger.AddLog(e.tp, "WS连接断开")
}
...
func (e *Event) OnMessage(socket *gws.Conn, message *gws.Message) {
	var msg common2.MessageData
	_ = json.Unmarshal(message.Bytes(), &msg)
	switch msg.OperationType {
	case "stop":
		var data = make(map[string]any)
		_ = msg.GetContent(&data)
		common2.Logger.AddLog(e.tp, data["reason"].(string))
		time.Sleep(time.Second * 2)
		common2.State.Set(e.key, false)
		e.WriteAny(socket, common2.NewMessage(msg.OperationId, msg.OperationType, nil))
		_ = socket.WriteClose(1000, nil)
	...
	case "forward_config":
		...
		e.WriteAny(socket, common2.NewMessage(msg.OperationId, msg.OperationType, map[string]any{
			"result": "success",
		}))
	case "host_config":
	...
	case "https_domain":
		...
		_ = msg.GetContent(&data)
		e.WsHttpsDomain(e.proxyBaseUrl, data)
		e.WriteAny(socket, common2.NewMessage(msg.OperationId, msg.OperationType, map[string]any{
			"result": "success",
		}))
	case "port_check":
		...
		_ = msg.GetContent(&data)
		e.WriteAny(socket, common2.NewMessage(msg.OperationId, msg.OperationType, e.WsPortCheck(data)))
	default:
		e.WriteAny(socket, common2.NewMessage(msg.OperationId, msg.OperationType, nil))
	}
}
