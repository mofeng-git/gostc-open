...
func (service *service) Page(req PageReq) (list []Item, total int64) {
	...
	if req.ClientOnline > 0 {
		...
		if req.ClientOnline == 1 {
			where = append(where, db.GostClientTunnel.ClientCode.In(onlineCodes...))
		} else {
			where = append(where, db.GostClientTunnel.ClientCode.In(offlineCodes...))
		}
	}
	...
	if req.NodeOnline > 0 {
		...
		if req.NodeOnline == 1 {
			where = append(where, db.GostClientTunnel.NodeCode.In(onlineCodes...))
		} else {
			where = append(where, db.GostClientTunnel.NodeCode.In(offlineCodes...))
		}
	}
