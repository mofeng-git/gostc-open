...
import {
  apiAdminGostClientProxyConfig,
  apiAdminGostClientProxyDelete,
  apiAdminGostClientProxyPage
} from "../../../api/admin/gost_client_proxy.js";
...
const pageFunc = async () => {
  try {
    ...
    let res = await apiAdminGostClientProxyPage(state.value.table.search)
  ...
  configRef.value.validate(async valid => {
    if (!valid) {
      try {
        ...
        await apiAdminGostClientProxyConfig(state.value.config.data)
        closeConfig()
        await pageFunc()
      } finally {
...
const deleteFunc = async (row) => {
  try {
    await apiAdminGostClientProxyDelete({code: row.code})
    searchTable()
  } finally {
  }
}
