...
import {
  apiAdminGostClientCreate,
  apiAdminGostClientDelete,
  apiAdminGostClientPage
} from "../../../api/admin/gost_client.js";
...
const pageFunc = async () => {
  try {
    ...
    let res = await apiAdminGostClientPage(state.value.table.search)
  ...
  createRef.value.validate(async valid => {
    if (!valid) {
      try {
        ...
        await apiAdminGostClientCreate(state.value.create.data)
        searchTable()
        closeCreate()
      } finally {
...
const deleteFunc = async (row) => {
  try {
    await apiAdminGostClientDelete({code: row.code})
    searchTable()
  } finally {
  }
}
...
const copyFunc = async () => {
  try {
    await copyToClipboard(state.value.look.key)
          ...
          <n-el class="client-item" tag="div" :style="{
            ...
            <n-space justify="end" style="width: 100%">
              <n-button size="tiny"
                        ...
                        type="info"
                        @click="router.push({name:'AdminGostClientLogger',query:{clientCode:row.code}})"
              >
                日志
              </n-button>
