...
import {
  ...
  apiAdminGostClientForwardPage
} from "../../../api/admin/gost_client_forward.js";
...
const pageFunc = async () => {
  try {
    ...
    let res = await apiAdminGostClientForwardPage(state.value.table.search)
...
const deleteFunc = async (row) => {
  try {
    await apiAdminGostClientForwardDelete({code: row.code})
    searchTable()
  } finally {
  }
}
...
const openMatcher = (row) => {
  state.value.matcher.matchers = JSON.parse(JSON.stringify(row.matchers))
  state.value.matcher.tcpMatcher = JSON.parse(JSON.stringify(row.tcpMatcher))
  state.value.matcher.sshMatcher = JSON.parse(JSON.stringify(row.sshMatcher))
...
<template>
  <div>
    ...
    <Modal
        ...
        confirm-text="关闭"
        cancel-text=""
        @on-confirm="closeMatcher"
        mask-close
    >
      <n-tabs>
        <n-tab-pane name="http" tab="HTTP协议">
          <div v-for="(matcher,index) in state.matcher.matchers" :key="index">
            <div style="font-weight: bold">
              规则{{ index + 1 }}
            </div>
            <div>内网目标：{{ matcher.targetIp + ':' + matcher.targetPort }}</div>
            <div>匹配域名：{{ matcher.host }}</div>
            <br>
          </div>
        </n-tab-pane>
        <n-tab-pane name="tcp" tab="TCP协议">
          <div>内网目标：{{ state.matcher.tcpMatcher.targetIp + ':' + state.matcher.tcpMatcher.targetPort }}</div>
        </n-tab-pane>
        <n-tab-pane name="ssh" tab="SSH协议">
          <div>内网目标：{{ state.matcher.sshMatcher.targetIp + ':' + state.matcher.sshMatcher.targetPort }}</div>
        </n-tab-pane>
      </n-tabs>
    </Modal>
