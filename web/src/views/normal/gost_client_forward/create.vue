...
import {apiNormalGostClientForwardCreate} from "../../../api/normal/gost_client_forward.js";
  ...
  createRef.value.validate(async valid => {
    if (!valid) {
      try {
        ...
        await apiNormalGostClientForwardCreate(state.value.data)
        back()
      } finally {
  ...
  try {
    ...
    if (state.value.nodes.length > 0) {
      for (let i = 0; i < state.value.nodes.length; i++) {
        if (state.value.nodes[i].forward === 1) {
          ...
          <n-grid x-gap="12" y-gap="12" cols="550:2 800:3 1400:4 1">
            <n-grid-item v-for="nodeItem in state.nodes">
              <n-alert
                  ...
                  @click="()=>{if (nodeItem.forward ===1) state.data.nodeCode = nodeItem.code}"
              >
                <n-radio
                    :key="nodeItem.code"
                    :value="nodeItem.code"
                    style="width: 100%;"
                    :disabled="nodeItem.forward !== 1"
                >
