...
import {apiNormalGostClientTunnelCreate} from "../../../api/normal/gost_client_tunnel.js";
  ...
  createRef.value.validate(async valid => {
    if (!valid) {
      try {
        ...
        await apiNormalGostClientTunnelCreate(state.value.data)
        back()
      } finally {
          ...
          <n-grid x-gap="12" y-gap="12" cols="550:2 800:3 1400:4 1">
            <n-grid-item v-for="nodeItem in state.nodes">
              <n-alert
                  ...
                  @click="()=>{if (nodeItem.tunnel ===1) state.data.nodeCode = nodeItem.code}"
              >
                <n-radio
                    :key="nodeItem.code"
                    :value="nodeItem.code"
                    style="width: 100%;"
                    :disabled="nodeItem.tunnel !== 1"
                >
