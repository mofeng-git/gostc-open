...
import {apiNormalGostClientLoggerPage} from "../../../api/normal/gost_client_logger.js";
...
const pageFunc = async () => {
  try {
    if (state.value.table.search.clientCode === '') {
      return
    }
    state.value.table.searchLoading = true
    let res = await apiNormalGostClientLoggerPage(state.value.table.search)
...
onBeforeMount(() => {
  state.value.table.search.clientCode = router.currentRoute.value.query.clientCode
  pageFunc()
})
